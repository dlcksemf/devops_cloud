...
def post_new(request: HttpRequest) -> HttpResponse:
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            post.save()
            messages.success(request, "성공적으로 저장 했습니다!")
...
def post_edit(request: HttpRequest, pk:int) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "성공적으로 수정 했습니다!")
            return redirect("diary:post_list")
    else:
...
def comment_new(request: HttpRequest, post_pk:int) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            # form.cleaned_data # 유효성 검사에 통과한 값들 -> 사전
...
# /diary/{{post_pk}}/comments/{{comment_pk}}/edit/
