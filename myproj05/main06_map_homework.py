...
# 멜론 TOP100 리스트에서 "곡명 / 단어수" 리스트 만들기
...
def title_and_length_for_song(song_dict):
    title: str = song_dict["title"]
    return [title, len(title)]


for title, length in map(title_and_length_for_song, song_list):
    print(title, length)
...
for title, length in map(title_and_length_for_song, filter(check_bts_song, song_list)):
    print(title, length)
